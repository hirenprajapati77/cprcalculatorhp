...
**Status:** Implemented via PR #137 (Retroactive sign-off recorded)
**Date:** 2026-08-13 (Updated 2026-08-17 / Retroactive Sign-off 2026-08-20)  
...
## Decision & Retroactive Sign-off Note

> **Note on Implementation (2026-08-17 / PR #137):**  
> The decision was effectively made and shipped to production via **PR #137** on 2026-08-17.
> 
> **What actually shipped in PR #137:**
> - **Cross-symbol ranking** is now **score-first** via `compareOvernightPickRows` (`overnightScore` descending, using `signalTime` as a tie-breaker). This ensures the highest quality setups win top-N placement.
> - **Per-symbol deduplication** across multiple rescans on the same day remains **freshness-first** via `compareLatestScanBySymbol` (`signalTime` descending). This ensures that for any single symbol, the latest scan's telemetry and metrics are used.
> 
> These are two distinct comparators serving two distinct purposes rather than a single monolithic toggle.

**Owner sign-off recorded 2026-08-17 (retroactive — see note):**

- [ ] (a) Keep Freshness Priority as-is (latest signalTime wins)
- [x] (b) Switch to Score Priority (highest overnightScore wins)

Owner: Hiren  Date: 2026-08-17 (retroactive — see note)
