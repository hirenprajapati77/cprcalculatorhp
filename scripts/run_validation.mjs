...
async function fetchHist(sym, days, interval) {
  ...
  for(let i=0; i<timestamps.length; i++) {
    if(quotes.open[i] !== null && quotes.close[i] !== null) {
      hist.push({ date: new Date(timestamps[i] * 1000).toISOString(), open: quotes.open[i], high: quotes.high[i], low: quotes.low[i], close: quotes.close[i] });
    }
  }
  return hist;
}

const getLastCompletedIndex = (history, isMonthly) => {
  const now = new Date();
  ...
  // Find the last index in the ORIGINAL array where the date is strictly before startOfCurrentPeriod
...
async function verify() {
  ...
  for (const sym of SYMBOLS) {
    try {
      ...
      const idxW = getLastCompletedIndex(wHist, false);
      const lastW = wHist[idxW];
      const wAtrPct = getAtrPct(wHist.slice(0, idxW + 1), lastW.close); // slice includes up to the completed candle
      const wCpr = calculateCPR(lastW, wAtrPct);
      
      const idxM = getLastCompletedIndex(mHist, true);
      const lastM = mHist[idxM];
      const mAtrPct = getAtrPct(mHist.slice(0, idxM + 1), lastM.close);
