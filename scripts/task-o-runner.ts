...
async function runTest(variant: string) {
  ...
  try {
    ...
    console.log(`Expectancy (Avg PnL): ₹${expectancy.toFixed(2)}`);
    ...
    for (const b of bands) {
      ...
      if (bTotal >= 15) {
        ...
        let bPnl = 0;
        bandTrades.forEach(t => bPnl += t.pnl);
        const bWinRate = (bWins / bTotal) * 100;
        const bExp = bPnl / bTotal;
        console.log(`Score >= ${b}: ${bWins}/${bTotal} (${bWinRate.toFixed(2)}%) | Exp: ₹${bExp.toFixed(2)}`);
      } else {
