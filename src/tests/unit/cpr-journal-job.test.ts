...
test('CPR Journal EOD Target Achieved handling', async (t) => {
  ...
  await t.test('skips trade with other staleness reasons even if target condition is numerically satisfied', async () => {
    // GAP_INVALIDATED: todayLow > entry * (1 + buffer) => entry unreachable
    // entry 100, todayLow 102, ltp 105, target 103 (target met, but gap invalidated)
    const mocks = mockJobDeps(
      [makeSignal({ symbol: 'GAP_FAIL', ltp: 105, entry: 100, target: 103, sl: 98, tc: 100, bc: 98, signalSummary: 'BULLISH,BREAKOUT,ABOVE_TC' })],
      { GAP_FAIL: { ltp: 105, low: 102, high: 106, open: 102, close: 105, previousClose: 101 } },
      choppyRegime
    );
    try {
      const result = await runCprJournalJob();
      assert.ok(result.skipped.some((s) => s.includes('GAP_INVALIDATED')), 'Gap-invalidated trade must still be skipped');
      assert.deepStrictEqual(result.logged, [], 'No trade must be logged when gap invalidated');
    } finally {
      mocks.restore();
    }
  });
