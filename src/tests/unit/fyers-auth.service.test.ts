...
test('FyersAuthService Diagnostic Logging', async (t) => {
  ...
  await t.test('direct call non-2xx status logs status and body text, then falls back', async () => {
    ...
    global.fetch = async (url: string | Request | URL) => {
      ...
      return { ok: false, status: 500, text: async () => 'Proxy failed' } as unknown as Response;
    };
    ...
    assert.ok(warnArgs.some(args => 
      String(args[0]).includes('Direct token exchange HTTP 403') && 
      String(args[0]).includes('IP not whitelisted')
  ...
  await t.test('direct call 200 with { s: "error" } logs full body, then falls back', async () => {
    ...
    assert.strictEqual(res.success, false);
    assert.ok(warnArgs.some(args => 
      String(args[0]).includes('Direct token exchange returned non-ok status') && 
      String(args[1]).includes('Invalid auth code')
  ...
  await t.test('direct call 200 with { s: "ok" } but missing token logs full body, then falls back', async () => {
    ...
    assert.strictEqual(res.success, false);
    assert.ok(warnArgs.some(args => 
      String(args[0]).includes('Direct token exchange succeeded but no token in response') && 
      String(args[1]).includes('missing_token')
