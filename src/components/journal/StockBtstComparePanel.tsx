...
export default function StockBtstComparePanel() {
  const { data, isLoading, error, refetch, isFetching } = useQuery({
    queryKey: ['stockBtstCompare'],
    queryFn: async () => {
      const res = await fetch('/api/journal/stock-btst-compare');
  ...
  if (error || !data?.success) {
    return <p className="text-sm text-accent-red font-mono">Failed to load stock BTST compare.</p>;
  }
  ...
  return (
    <div className="space-y-4 font-mono text-xs">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-text-primary">Stock BTST — Live vs Backtest</h2>
          <p className="text-[11px] text-text-secondary mt-1">
            Live = option CMP journal (CE/PE). Backtest = stock spot proxy from latest completed BTST_STBT_DRIVEN run.
          </p>
      ...
      {rows.length === 0 ? (
        <p className="text-text-secondary py-8 text-center">
          No stock BTST journal entries or backtest trades yet.
        </p>
      ) : (
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead className="text-[10px] text-text-tertiary uppercase bg-bg-secondary/60 border-b border-border-primary">
              <tr>
                ...
                <th className="px-3 py-2">VDU / Regime</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row: {
                ...
                <tr key={`${row.symbol}_${row.signalDate}`} className="border-b border-border-primary/30 hover:bg-bg-secondary/30">
                  ...
                  <td className="px-3 py-2">{row.live?.direction ?? row.backtest?.direction ?? '—'}</td>
                  ...
                  <td className="px-3 py-2 text-text-secondary">
                    {row.backtest
                      ? `${row.backtest.vduBand ?? '—'} / ${row.backtest.regimeTrend ?? '—'}`
                      : '—'}
                  </td>
                </tr>
              ))}
