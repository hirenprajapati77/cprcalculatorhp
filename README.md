...
- **31 Aug — Production Validation: ETF Exclusion Gaps & Database Migration Fixes (PR #150 / Commit `441d5123` & `441d5123`)**: Closed ETF scanner leaks (HDFCMOMENT, MONQ50, LICNMID100, MULTICAP) and resolved a regex MID-prefix regression. Dropped redundant DailyOhlcv index via database migration. Resolved PostgreSQL date comparison type mismatch crash (`text >= timestamp without time zone`) and corrected unit test baselines to align with the tightened 1.5% extension limits.
