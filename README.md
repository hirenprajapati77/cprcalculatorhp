...
- **04 Sep — 2-Month Deep Code Review: 52 Defects Resolved across Tiers 1–4 (PRs #161, #162, #163, #164)**:
  ...
  - **Tier 2 (19 HIGH / PR #162)**: Relaxed Rule 5 inequality (`>=` / `<=`) restoring up to 20 points on extreme closes (`btst-ranking.service.ts`); increased discovery timeout to 180s; stripped `:NSE` exchange suffix in morning journal option queries; corrected Category B intraday index max score cap to 25 pts (full 100-pt scale); added null fallback for `stock.sector`; bounded CTE date window to 250 days; guarded recent IPOs from false ATH labels; filtered ETFs from market breadth; synchronized client extension UI gate to 2.5%; expanded option symbol regex for weekly index and BSE Sensex; corrected VPA climax detection on red shooting stars; added 5s timeout to Cloudflare proxy; eliminated redundant empty `DELETE` queries; fixed watchlist triple-fetch; added unique UUID lock tokens and TTL in-memory fallback; suppressed aggressive PM2 restarts during active trading sessions (`IN_SESSION=1`); tuned Node.js heap cap (`--max-old-space-size=384`) and PM2 restart threshold (`480M` in `ecosystem.config.js` / `650M` production headroom via `ops/ecosystem.config.cjs` and `ops/deploy.ps1`) for 1GB VM capacity.
  - **Tier 3 (15 MEDIUM / PR #163)**: Classified winning trades opening weak as `MODEL_VALID` instead of `GAP_FAILURE` when `pnlPct >= 0`; added `direction` to `IndexBtstCompareRow`; guarded intraday index scanner against inverted CPR bands (`risk <= 0`); fixed sector lookup hyphen handling for `BAJAJ-AUTO` and sanitized `NaN` change percentages; verified Bhavcopy numeric sanitization; re-ordered cache resolution to query Redis *before* in-memory cache; calculated monthly option expiry to true last Thursday; escaped and truncated Telegram errors (1,500 char cap); synthesized true `atrPct` in synthetic stock candles; enforced forward target selection strictly ahead of current LTP; removed redundant index on `DailyOhlcv.date`; added composite index on `TradeJournal(tradeDate, signalType)`; added `take: 2000` query safety limit; implemented constant-time SHA-256 token comparison; added bounded retry strategy to secondary Redis client.
  - **Tier 4 (7 LOW / PR #164)**: Deterministic zero-padded IST time string formatting; CSV formula injection defense (CWE-1236) prepending `'` on unsafe prefixes while preserving numbers; shared Prisma singleton from `@/lib/db` and configurable `BACKFILL_MIN_ROWS`; capped stop-loss risk to max 3.0% of entry on opening flash wicks; removed redundant `countdown` state and throttled `useBtstState` timer from 1s to 30s; made deployment connection credentials configurable via environment variables and replaced `StrictHostKeyChecking=no` with `accept-new`; added `npm run typecheck` (`tsc --noEmit`) step prior to building in `deploy.ps1`.
  - Tests: **798 total tests (797 pass, 1 skip, 0 fail)**.
- **03 Sep — Core Quality, Momentum Leaders Hardening & Safety Controls (PRs #155, #156, #158, #159)**:
  - **Circuit Lock Detection & Badging (PR #156)**: Added `detectCircuitLock()` to Momentum Leaders with $\pm 0.20\%$ tolerance around 5%, 10%, 20% NSE circuit limits; surfaced high-visibility `🔒 Limit Up` / `🔒 Limit Down` badges in table rows, audit drawer, and CSV export.
  - **PrevClose Validation & Corruption Immunity (PR #158)**: Hardened `computeCompoundedReturn` to return `null` on invalid `prevClose` ($\le 0$, `null`, `NaN`); enforced single-sourced whole-stock exclusion across 1D/5D/10D/21D return windows with ops logging. Empirically verified on live production DB (77,551 candles across 2,725 symbols): 0 corrupted rows, guaranteeing full mathematical immunity without leaderboard shrinkage.
  - **Deterministic Ranking & Sort (PR #159)**: Implemented standard competition ranking in `rankWindow` (tied returns share rank and percentile) and added secondary symbol tie-breaker (`a.symbol.localeCompare(b.symbol)`) to eliminate sort volatility across refreshes.
  - **Historical Query Bounding (PR #159)**: Added `AND date >= ${oldestDate}` to Market Breadth `RankedHistory` CTE, pruning table scans beyond 250 trading days as `DailyOhlcv` expands.
  - **Overnight Fail-Closed Regime Gate (PR #159)**: Added fail-closed check (`regime.reliable === false`) to suppress overnight candidate signal emission when Nifty market feed is unavailable.
  - **Scanner Direction & Extension Gating (PR #155)**: Attached inferred setup direction (`LONG`/`SHORT`) and rendered `TARGET MET` badge in `ScannerClient.tsx`; refined extension evaluation to active breakout setups with 2.5% threshold.
  - Tests: **769 total tests (768 pass, 1 skip, 0 fail)**.
- **02 Sep — Multi-Window Momentum Leaders Scanner with Universe Toggle (`ALL_NSE` vs `NSE_FNO`)**: Shipped new institutional scanner (`/market-tools/momentum-leaders`) computing corporate-action-safe compounded returns across 1D, 5D, 10D, and 21D trading windows. Features a ₹10 Cr 20-day average turnover base liquidity floor (`MIN_LIQUIDITY_TURNOVER_CR = 10.0`), bounded additive composite scoring formula ($S \in [0, 100]$), independent percentile ranking ($p = \frac{N - rank}{N - 1} \times 100$) per universe, dual universe switcher (`F&O Universe` vs `ALL NSE`), Redis caching, daily 19:15 IST precompute cron, and RFC 4180 CSV export. Tests: **754 total tests (753 pass, 1 skip, 0 fail)**.
- **02 Sep — 10-Day Deep Code Review: Stability & Resilience Fixes**: Addressed high-priority findings from comprehensive code review. **Critical**: Attached silent catch `promise.catch(() => {})` in `withTimeout.ts` preventing unhandled promise rejections and Node worker crashes on late timeouts. **High**: Implemented safe message pagination (3,900 char ceiling) in `sendBtstAlert` (`telegram.service.ts`), preventing message drops when combined setups exceed Telegram's 4,096-character limit; added a 60-second periodic active sweeper (`sweepExpiredMemoryCache`, unreferenced) in `redis.ts` to actively evict expired keys from in-memory cache and fixed glob `?` wildcard matching in `delPattern`. **Medium/Tests**: Added unit test coverage for zero-range (`high === low`) division safety in CLV index ranking. Tests: **753/754 pass**, 0 failures.
- **01 Sep — Bull Flag & Pole (High Tight Flag) Pattern Detection in Market Tools**: Integrated institutional Bull Flag & Pole pattern recognition into the 52W Pattern Breakout Scanner. Detects explosive prior advances ($\ge 15\%$) followed by tight, controlled consolidations ($\le 12\%$ pullback, volume dry-up $\le 0.85\times$). Ranks at the top of the deterministic pattern hierarchy with dedicated cyan badges and quick-filter tabs. Tests: **726/727 pass**, 0 failures.
- **01 Sep — Export to Excel/CSV & Print-Ready PDF across Market Tools**: Added client-side export capability to 52W Pattern Breakouts, Multi-Year Breakouts, and Market Breadth scanners. Exports RFC 4180 CSV with UTF-8 BOM (`\uFEFF`) for zero-formatting-loss Excel compatibility (preserving INR ₹ currency, percentages, and floating-point values). Added dedicated `@media print` layout styles for presentation-ready document printing. Tests: **724/725 pass**, 0 failures.
- **01 Sep — Volume Price Analysis (VPA) in Market Tools**: Integrated Close Location Value (CLV), 20D RVOL, and footprint classification into 52W Pattern Breakout and Multi-Year Breakout scanners. Detects and tags institutional footprints: 🟢 `Volume Confirmed` (+5 pts), 🔵 `Supply Absorption` (+5 pts), 🔴 `Upper Wick Trap / Climax Reject` (-10 pts), and 🟡 `Low Volume` (-3 pts). Surfaces VPA status chips in table views and detailed footprint analytics in pattern drawers. Tests: **721/722 pass**, 0 failures.
- **01 Sep — 1-Month Deep Code Review: 11 Issues Resolved across Domains 1–4**: Exhaustive line-by-line review covering all 303 commits across the 30-day period (Aug 1 – Sep 1, 2026) resolved 11 issues across Overnight/Index Engine, Market Tools & Bhavcopy Ingestion, Realtime Scanner/Alerts, and Infrastructure/Watchdogs. **Critical**: Restored Index BTST/STBT discovery before 15:15 IST by conditionally evaluating Rule 5 liquidity points; fixed scheduler claim lock release on `TimeoutError` to eliminate duplicate hanging background jobs and OOM leaks; added bounded date filtering to CTE window queries across 626K+ rows in `DailyOhlcv`. **High**: Corrected inverted option theta risk buffer math (`1 - thetaBuffer`), ensuring 10% tighter SL on expiry week ($\le 4$ DTE); fixed `resolvePreviousClose` clock bypass in historical backtests; added `inFlightCompute` deduplication to `PatternBreakoutService`; added NaN sanitization to `bhavcopy-ingest.ts`; updated `ops/mem_watchdog.sh` to protect `market_tools:*` and `market_breadth:*` Redis keys from off-hours pruning. **Medium/Low**: Added Telegram breakout alert fallback to `TELEGRAM_CHAT_ID`; added automated pruning for BTST retry count maps; expanded flat base detection window to 45 candles; added unmount `AbortController` cancellation to client-side refresh. Tests: **715/716 pass**, 0 failures.
- **01 Sep — 10-Day Deep Code Review: 7 Bug Fixes (Pass 2)**: Comprehensive review covering all 91 commits across the 10-day period (Aug 22 – Sep 1, 2026) resolved 7 confirmed issues across 4 domains. **Critical**: Fixed undeclared `hostname` variable in `server-starter.js` causing fatal ReferenceError on Node startup. **High**: BullMQ queue connection parses `REDIS_URL` properly; `overflow-x: hidden` replaced with `overflow-x: clip` in `globals.css` restoring `position: sticky` on desktop navbar; added `ltp <= 0` guard in `btst-alert.job.ts` preventing false GAP_FAILURE_EXIT alerts during market open feed delays. **Medium**: Fixed 24h `tradeDate` timestamp mismatch in Gap Failure journal updates and added holiday-aware trading session resolution via `getISTTime()`; added `.finite()` to `CPR_WEIGHT` in Zod schema; updated `sendRawMessage` to default to `TELEGRAM_GROUP_CHAT_ID`. **Low**: Fixed Cup & Handle tightness bonus depth threshold in `pattern-breakout.service.ts`. Tests: **714/715 pass**.
- **31 Aug — 10-Day Deep Code Review: 25 Bug Fixes (PR #151 / Commit `9381c945`)**: Systematic deep review of all ~70 commits in PRs #141–#149 found and fixed 25 bugs. **Critical (5)**: Prisma `$queryRaw` Decimal comparisons silently false (market breadth permanently zero), two unauthenticated refresh-DoS vectors on breadth/breakout routes, INDEX kill switches leaving stale signals in DB, unit tests testing their own local mocks instead of real service. **High (7)**: precompute job OR→AND success logic (masks sub-job failures), Zod `coerce().optional()` NaN bug, Telegram 4096-char truncation (chunking), dangling setTimeout in scanner, backtest `evaluateExtension` using live clock, Next.js `0.0.0.0` hostname poisoning canonical URLs, CI tests crash before running (missing env vars). **Medium (9)**: 20-day return off-by-one candle, BE/SM stocks excluded from bhavcopy before dedup, AbortController cleanup on breadth/breakout pages, PM2 missing log file paths, missing `[date, symbol]` DailyOhlcv cross-sectional index, Friday gate test coverage, iOS PWA landscape void. **Low (4)**: themeColor in Next.js viewport, edge-case tests for NaN/null candle prices. Tests: **709/710 pass**. **Action required on deploy:** `npx prisma migrate dev --name add_date_symbol_idx` (B19 index).
- **31 Aug — Production Validation: ETF Exclusion Gaps & Database Migration Fixes (PR #150 / Commit `441d5123`)**: Closed ETF scanner leaks (HDFCMOMENT, MONQ50, LICNMID100, MULTICAP) and resolved a regex MID-prefix regression. Dropped redundant DailyOhlcv index via database migration. Resolved PostgreSQL date comparison type mismatch crash (`text >= timestamp without time zone`) and corrected unit test baselines to align with the tightened 1.5% extension limits.
